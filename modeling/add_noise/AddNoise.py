...
class AddNoise(nn.Module):
    ...
    def NoiseToImage(self, image_shape, noise, targets, device):
            ...
            # (4, 171, 77, 512) -> (4, 171, 512, 77, 1)
            noise = noise.view(-1, 512, 77, 1)  # 变为 (4 * 171, 512, 1, 1)
            noise = F.interpolate(noise, size=(12, 12), mode='bilinear',align_corners=False)
            # (4 171, 512, 77, 1) -> (4 171, 512, 12, 12)
            noise = self.Conv1NoiseToImage(noise)
            # (4171, 512, 12, 12) -> (4 171, 768, 12, 12)
            ...
            del zero_tensor,new_mask,targets
